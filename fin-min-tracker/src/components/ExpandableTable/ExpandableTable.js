    ...
    if (this.props.budget) {
      ...
      const dynamicRows = finances.map(title => {
        if ('tags' in budget[title]) {
          return (
            <tr key={title}  >
              <td>{title}  <svg onClick={() => this.rowClickedHandler(title)} role='button' width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-info-circle" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
